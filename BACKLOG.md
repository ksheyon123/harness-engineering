...
## ~~#2 · QA 에이전트가 "테스트 러너 없음"을 전제한다~~ <sub>P1</sub> ▸ **완료** (`b78e9ef`)

> **[`harness/drop-phase1/spec.md`](./harness/drop-phase1/spec.md) 로 이관 후 구현 완료.** 사실 주장을 지우고 **조건부 규칙**으로 바꿨다 — 사실은 시간이 지나면 틀리지만 규칙은 안 틀린다.
>
> - `qa.md` 의 'Phase 1 주의' 절 → **'테스트 러너가 있는지 먼저 확인한다'**. QA 는 `harness/config.json` 을 Read 해 `gate.test` 로 러너 유무를 판단하고(Bash 가 없어 게이트를 실행할 수 없으므로 `--list` 가 아니라 설정 파일을 읽는다), 항목이 있으면 `testFilePatterns` 로 테스트 파일을 실제로 읽어 대조한다. 없을 때만 `❌ 누락(테스트 러너 없음)` 을 근거와 함께 기록한다.
> - `harness-engineering.md` — §6 Phasing 표를 **게이트 대상의 단일 출처** 서술로 대체(이력은 [설계 변경 이력] 에 남겼다), §7.3 의 pre-push 개념 스케치를 실제 구현(게이트 위임 + 트리 해시 조건부 스킵 + 해시 스킵 + 패키징 게이트)과 맞춤, §10 의 '테스트 러너 = Phase 1' 결정을 '도입 완료' 로, §11 적용 순서를 현재 프로토콜(config → worktree → test-first → pre-commit/pre-push)로 갱신.
> - `README.md` 의 '알려진 제약' 두 줄(#2·#7)은 둘 다 해소돼 제거하고, spec 목록에 이후 task 4개를 추가했다.
>
> **범위 밖 명시**: 종료된 옛 task 의 `harness/<task>/spec.md`·`qa-checklist.md` 에 남은 'Phase 2 보류' 서술은 **그 시점의 기록**이라 손대지 않았다. 그것들은 지시가 아니라 이력이다.

**증상(당시)** — 이건 문서 오타가 아니라 **동작하는 오지시**다. `qa.md` 가 QA 에게 커버리지를 전부 `❌ 누락(테스트 없음)` 으로 적으라고 지시하고 있어서, 테스트가 있는 프로젝트에서도 QA 가 잘못된 매트릭스를 쓸 수 있다.
...
| ✔ | ~~#2 Phase 1 서술 제거~~ | **완료** `b78e9ef` | QA 에게 '커버리지 전부 ❌' 를 지시하던 오지시를 조건부 규칙으로 대체 |
| — | #6 doggy rules | 대기 | 별도 저장소, 독립적으로 가능. **이 저장소에 남은 유일한 항목** |
