...
### 기능: 훅의 종료 코드를 출력 성공 여부와 분리

> 이 기능은 구현 중 발견된 결함에서 나왔다. 위 `pre-commit`/`pre-push` 기능을 구현한 뒤 검증하다가, **막아야 할 커밋이 조용히 통과하는** 경우를 재현했다.

- **의도**: 훅의 차단 여부가 "진단 메시지를 끝까지 출력하는 데 성공했는가"에 의존하고 있었다. 검사 결과와 무관한 조건이며, 실패 방향이 최악이다 — 게이트가 막아야 할 커밋이 **아무 경고 없이 통과**한다.

  재현: 훅의 stdout 이 조기에 닫히는 파이프에 연결되면(`git commit ... | head -6`, `| grep -q` 등 EOF 전에 나가는 소비자), 훅이 진단을 출력하다가 SIGPIPE 로 죽는다. `exit 1` 문에 도달하지 못하므로 거부 의사가 git 에 전달되지 않는다. 훅 실행 경로를 로그로 추적해 **어느 `exit` 문에도 도달하지 못했음**을 확인했다.

  | 훅 | stdout 정상 | `\| head -3` | `\| head -1` |
  |---|:--:|:--:|:--:|
  | 직접 출력 | 1 ✅ | 141 ❌ | 141 ❌ |
  | 출력을 서브셸로 감쌈 | 1 ✅ | 1 ✅ | 1 ✅ |

- **방식**: 훅 **자신의** 진단 출력만 서브셸로 감싼다. 서브셸은 별개 프로세스라 SIGPIPE 로 죽어도 부모(훅)는 비0 상태를 받을 뿐이고, 부모는 아무것도 쓰지 않았으므로 `exit` 문까지 도달한다. 두 훅 모두에 헬퍼를 둔다.

  ```sh
  say() { ( printf '%s\n' "$@" ) >&2 || true; }
  ```

  `>&2`(stderr)는 관례이자 이중 보험이다 — 호출자가 stdout 만 파이프로 받는 흔한 경우엔 애초에 파이프에 닿지 않는다.

- **주의**:
  - **`gate.mjs` 의 `stdio: "inherit"` 은 그대로 둔다.** vitest/tsc 출력은 훅이 복사하는 것이 아니라 라이브로 흐르는 원본이다. 파일로 빼고 꼬리를 재출력하면 없던 중복이 생기고, 실패 원인을 세션이 바로 볼 수 없게 된다.
  - 파이프가 닫힌 상태에서 vitest 가 SIGPIPE 로 죽으면 `gate.mjs` 는 그것을 게이트 실패로 판단한다. 게이트가 실제로는 통과했을 수 있으므로 **거짓 거부**지만, 방향이 안전하다(막는 쪽). 이 동작을 유지한다.
  - `trap '' PIPE` 로도 해결되지만 쓰지 않는다 — 자식 프로세스(node 등)의 SIGPIPE 처리까지 바뀌어 부작용 범위가 넓다. 서브셸은 훅 자신에게만 국한된다.
  - 정보성 출력(차단과 무관한 진행 메시지)도 같은 헬퍼를 쓴다. 그것 때문에 훅이 죽으면 통과해야 할 커밋/push 가 막히는 **거짓 차단**이 난다.

- **인수기준**: 지켜야 할 성질은 **"막아야 할 커밋이 통과하지 않는다"** 하나다. 출력이 잘리는 상황에서 통과해야 할 커밋이 막히는 것(거짓 차단)은 이 설계가 **의도적으로 받아들이는 비용**이다 — `stdio: inherit` 을 유지하는 한 vitest 가 SIGPIPE 로 죽어 게이트 실패로 판정되는 것을 피할 수 없고, 방향이 안전하기 때문이다.

  | 게이트 | 출력 소비자 | 기대 |
  |---|---|:--:|
  | 실패 | 정상 | 차단 ✅ |
  | 실패 | `\| head -1` | **차단 ✅ (이 수정의 핵심)** |
  | 통과 | 정상 | 커밋 ✅ |
  | 통과 | `\| head -1` | 차단 ⚠️ (거짓 차단 — 허용) |

  - 훅이 **거부를 결정한 뒤 그 결정을 잃는 경로가 없다**. 즉 훅은 어떤 출력 상황에서도 자기 `exit` 문에 도달한다.
  - 출력을 EOF 까지 읽는 소비자(`| tail`, 리다이렉션, 터미널)에서는 거짓 차단이 발생하지 않는다.
