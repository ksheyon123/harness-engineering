...
- 자기 코드의 테스트는 개발자가 작성한다. 테스트 러너가 있는 패키지에서 작성한다(아래 '검증 명령' 참고).
...
- spec 전체 구현이 끝나면 다음을 순서대로 **자동** 수행한다: **① 객관 게이트(타입 검사 + 테스트) → ② 통과 시 QA 체크리스트 생성 → ③ 자동 커밋 → ④ 작업 브랜치로 자동 push**(아래 각 절 참고). 그 뒤 변경 요약 + 게이트 결과 + QA 결과 + 커밋 해시 + push 결과를 한 번에 보고한다. **체크박스 확정·PR 리뷰·머지는 사람의 몫이다.**
...
### 자동 커밋 + push (게이트 통과 시)
- **커밋 조건**: 스코프의 모든 검증 명령(타입 검사 + 해당 패키지 테스트)이 **전부 통과**할 때만 커밋한다. 하나라도 실패하면 커밋하지 않고 멈춰 보고한다.
...
- **자동 push**: 커밋 후 **현재 작업 브랜치로 push 한다**(`git push -u origin <현재-브랜치>`). push 도중 pre-push 훅이 객관 게이트를 재실행하고, 위에서 주입한 `input_hash`가 일치하면 QA를 스킵한다.
...
> **도입 프로젝트가 채운다.** 아래는 형식 예시다. 여기 적은 명령이 곧 1층 객관 게이트이며, `.githooks/pre-push`의 `TSC_DIRS`/`TEST_DIR` 과 **같은 대상**을 가리켜야 한다(둘이 어긋나면 세션에서는 통과한 코드가 push에서 막힌다).

```
- <패키지 경로>: <타입 검사 명령>, <테스트 명령>
```

테스트 러너가 있는 패키지에서 자기 코드 테스트를 test-first 로 작성한다. 러너가 없는 패키지는 그 사실을 여기에 명시한다 — QA 가 커버리지 열을 '테스트 없음'으로 기록하는 근거가 된다.
