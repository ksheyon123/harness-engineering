...
역할 기반(기획자 / 개발자 / QA) 문서 주도 개발 하네스. 설계 전문은 `README.md` 참고.
...
**`main` 체크아웃 세션은 디스패처다.** 거기서 하는 일은 요구사항 논의와 브랜치명 확정, 그리고 `node scripts/worktree-add.mjs <branch> --launch` 실행뿐이다. **기획(planner 스폰)을 포함한 실제 작업은 전부 worktree 세션이 한다** — spec·index·코드·테스트·qa-checklist 가 같은 브랜치 위에 쌓여야 `main` 직접 커밋이 사라지고 planner 의 "브랜치에서 task 유도" 규칙이 성립한다. worktree 세션은 spec 이 아직 없으면 planner 를 먼저 스폰하고, 그대로 이어서 구현한다.
...
- 자기 코드의 테스트는 개발자가 작성한다. 어디에 쓸지는 `node scripts/gate.mjs --list` 로 확인한다(아래 '검증 명령' 참고).
- 동작 변경은 각 기능의 **인수기준 범위 안에서만** 한다.
- **문서 먼저(doc-before-code)**: **동작(인수기준)이 바뀌는** 변경은 코드보다 spec/문서를 먼저 확정한다(planner 또는 직접) — 새 기능·리비전·후속 수정·하네스 자체 수정 모두. 인수기준이 바뀌지 않는 변경(순수 리팩터, 오타 수정, 이미 있는 문서 문구 다듬기)은 예외다.
...
- spec 전체 구현이 끝나면 다음을 순서대로 **자동** 수행한다: **① 객관 게이트(`node scripts/gate.mjs`) → ② 통과 시 QA 체크리스트 생성 → ③ 자동 커밋 → ④ 작업 브랜치로 자동 push**(아래 각 절 참고). 그 뒤 변경 요약 + 게이트 결과 + QA 결과 + 커밋 해시 + push 결과를 한 번에 보고한다. **체크박스 확정·PR 리뷰·머지는 사람의 몫이다.**
