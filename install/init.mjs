 ...
 *
 * 그래서 **worktree 안에서 필요한 것은 사본에 실체로 있어야 한다.** 훅 본체는 패키지에
 * 두고, A 에는 **한 줄짜리 shim** 을 남긴다 — node 의 상향 해석이 `A/node_modules` 까지
 * 올라가 본체를 찾는다(실측).
 *
 * **사본에 도달하는 길은 둘이다.** 커밋되어 git 이 데려가거나, `post-checkout` 이 심거나.
 * 한때는 앞엣것뿐이라 설치기가 `git add -f` 로 A 의 무시 규칙을 뚫었는데, **하네스 파일을
 * 커밋할지는 A 가 정할 일**이라 그건 설치 도구의 몫이 아니었다. 지금은 뚫지 않는다.
 *
