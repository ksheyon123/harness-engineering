...
**`init` 이 끝에서 `smoke` 를 직접 돈다.** 그래서 따로 부를 필요가 없다 — 배선이 깨졌으면 그 자리에서 종료 코드 1 이다. 다만 **사본 도달만 안 된 것은 1 이 아니다.** `init` 은 남의 저장소에 커밋을 만들지 않으므로 그것으로 실패를 내면 종료 코드가 *항상* 1 이 되고, 항상 같은 값은 아무 정보도 아니다.
...
### 커밋은 설치 절차가 아니다 — 도달 경로 둘 중 하나일 뿐이다

서브에이전트는 `isolation: worktree` 로 자기 사본에서 돈다. 훅·규약·에이전트 정의가 그 사본에 **없으면 막히는 게 아니라 그냥 통과한다** — 층 1 이 통째로 사라지는데 아무 신호도 없다. 그래서 물어야 할 것은 "커밋됐는가" 가 아니라 **"사본에 도달하는가"** 이고, 길이 둘이다:

| 경로 | 무엇이 데려가나 | 조건 |
|---|---|---|
| **심기** | `post-checkout` 이 본체 디스크에서 복사한다 | `core.hooksPath` 가 **절대경로**로 서 있고 `.githooks/post-checkout` 이 있다 — `init` 이 둘 다 세운다 |
| **커밋** | git — 사본은 `HEAD` 에서 만들어진다 | 필요한 것이 전부 `HEAD` 에 있다 |

**심기는 git 상태를 묻지 않는다.** 본체의 워킹트리에서 복사하므로 무시됐든 미추적이든 스테이징만 됐든 똑같이 데려간다 — **없는 것만 못 데려간다.** 그래서 `init` 만 돌리고 커밋하지 않아도 정상 경로다.

`harness smoke` 가 이것을 마지막 항목으로 묻는다. 심기가 서 있으면 커밋 없이도 초록이다:

```
  ✓  worktree 안에서도 살아남는다 — 필요한 것이 사본에 도달한다
      20개가 도달한다 — `post-checkout` 이 사본이 만들어질 때 심는다.
```

빨간불이 남는 것은 **둘 다 없을 때**뿐이고, 그때 ✗ 가 상태별 처방을 직접 찍는다:

| 상태 | 다음에 칠 것 |
|---|---|
| 파일이 없다 | `harness init` 부터 — 심기도 커밋도 없는 파일은 못 데려간다 |
| `.gitignore` 가 막는다 | **뚫지 마라.** `.claude/` 를 무시한 것은 그것을 개인 설정으로 본 판단이다 — `post-checkout` 을 배선하면 커밋 없이 심긴다 |
| 아직 안 담겼다 | `git add -A` 로 담아 커밋하거나, `post-checkout` 을 배선한다 |
| 스테이징까지만 됐다 | **커밋해야** 사본에 간다 — 기준은 인덱스가 아니라 `HEAD` 다(실측: `git add` 만 한 상태의 사본에는 `.claude` 가 통째로 없다) |

> **`core.hooksPath` 는 절대경로여야 한다.** git 은 이 값을 **각 워킹트리 최상단 기준**으로 푸므로, 상대값(`.githooks`)은 갓 만들어진 사본 안에서 *그 사본의* `.githooks/` 를 가리키는데 커밋되지 않은 저장소에는 거기 아무것도 없다 — 훅이 실패하는 게 아니라 **안 불린다.** 그러면 심기가 안 돌고, 심기가 안 도니 `.githooks/` 가 생기지도 않는다. 순환이다. `init` 이 절대경로를 심어 그것을 끊는다.

> 하네스를 팀 규약으로 삼겠다면 커밋한다. 그 첫 커밋에서 `pre-commit` 이 막을 수 있다 — 보호 브랜치(`main`/`dev`/`master`)에 직접 커밋하려 할 때다. **층 2 가 살아 있다는 증거이므로 정상이다.** `git switch -c chore/harness` 로 브랜치를 자르고 커밋해라.
...
**`.claude/harness.config.json`** 에 두고, **달라지는 값만** 적는다. 없으면 기본값으로 돈다.

> **루트의 `harness.config.json` 도 읽히지만 그건 fallback 이다.** 이미 설치된 저장소가 거기 파일을 갖고 있어서 계속 읽어 준다 — 안 읽으면 조용히 기본값으로 돌아가고, 남의 저장소에서 기본값은 곧 틀린 값이다. 둘 다 있으면 **`.claude/` 쪽이 이기고**, `harness doctor` 가 옮기라고 말한다.
>
> `.claude/` 아래로 모으는 이유: 하네스가 만드는 것이 **한 접두어 아래** 있어야 A 가 `.gitignore` 에 쓴 한 줄이 전부를 덮는다. 루트에 흩어져 있으면 `.claude/` 는 무시하기로 했는데 설정 파일만 `git add -A` 에 잡히는 식으로 A 의 결정이 샌다. 커밋 여부는 여전히 A 가 정한다 — 하네스는 **결정할 대상을 한 덩어리로 만들 뿐**이다.
...
| `smoke` 가 `필요한 것이 사본에 도달한다` 로 ✗ | **도달 경로가 둘 다 없다는 뜻이다.** 처방은 그 ✗ 가 상태별로 직접 찍는다 — 먼저 `core.hooksPath` 가 **절대경로**인지부터 봐라(상대값이면 사본에서 훅이 안 불려 심기가 통째로 죽는다). 그것이 서 있으면 커밋은 필요 없다 |
| `.gitignore` 에 `.claude` 가 있는 저장소에 설치했다 | **정상이다. `init` 은 인덱스를 건드리지 않는다** — 무시해 둔 것은 그것을 개인 설정으로 본 판단이고, `post-checkout` 심기가 그 상태로도 사본에 데려간다 |
| `init` 이 종료 코드 1 | 배선이 깨진 것이다 — 뒤에 붙은 `smoke` 출력의 ✗ 를 봐라. **사본 도달만 안 된 것은 1 이 아니다**(그건 0 + 안내다) |
